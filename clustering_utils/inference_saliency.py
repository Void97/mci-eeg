...
GRADIENT_METHODS = [
    'vanilla', 'input_x_gradient', 'smoothgrad',
    'smoothgrad_sq', 'vargrad', 'integrated_gradients',
]
...
def inference(dataset_name, task, model, samples, targets, groups,
    ...
    for fold, test_index in fold_splits.items():
        ...
        test_loader   = DataLoader(
            TensorDataset(test_samples, test_targets, test_groups),
        ...
        weights_path = (
            f'results/weights/{dataset_name}/'
        ...
        fold_gradients, fold_preds, fold_labels, fold_subject_ids = [], [], [], []
        for bx, by, bg in test_loader:
            bx = bx.to(device).clone().detach().requires_grad_(True)
            by, bg = by.to(device), bg.to(device)

            preds = net(bx).argmax(dim=1)
            ...
            fold_gradients.append(
                _attribute(saliency_inst, gradient_method,
                           bx, by.detach().cpu().long().tolist())
                .detach().cpu().numpy()
            )
        ...
        if fold_gradients.shape[1] == 1:
            fold_gradients = fold_gradients.squeeze(axis=1)
        print(f"Fold {fold + 1} gradients shape: {fold_gradients.shape}")

        correct_subjects, correct_negative = [], []
        av_pos, av_neg = [], []
        for id_ in np.unique(fold_subject_ids):
            mask       = fold_subject_ids == id_
            true_label = int(fold_labels[mask][0])
            pred_label = Counter(fold_preds[mask]).most_common(1)[0][0]
            if pred_label == target_class and true_label == target_class:
                av_pos.append(fold_gradients[mask].mean(axis=0))
                correct_subjects.append(id_)
                subject_fold_map[int(id_)] = fold
            elif pred_label == negative_class and true_label == negative_class:
                av_neg.append(fold_gradients[mask].mean(axis=0))
                correct_negative.append(id_)

        print(f"Fold {fold + 1}: {len(correct_subjects)} positive, "
              f"{len(correct_negative)} negative correctly predicted")

        gradients_list.append(np.array(av_pos))
        negative_gradients_list.append(np.array(av_neg))
    ...
    print(f"Saliency maps: {gradients_list.shape}  |  "
          f"Correctly predicted subjects: {subject_ids_list.shape}")

    np.save(
        os.path.join(
            save_dir,
            f'{dataset_name}_{task}_{model["name"]}_iteration_{best_iteration}'
            f'_{gradient_method}_saliency_maps.npy',
        ),
        gradients_list,
    )

    return (gradients_list, negative_gradients_list,
            subject_ids_list, negative_subject_ids_list,
            subject_fold_map)
