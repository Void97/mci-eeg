...
class TSConv(nn.Sequential):
    def __init__(self, num_ch, F, C1, C2, D, P1, P2, Pc) -> None:
        super().__init__(
            nn.Conv2d(1, F, (1, C1), padding='same', bias=False),
            nn.BatchNorm2d(F),
            nn.Conv2d(F, F*D, (num_ch, 1), groups=F, bias=False),
            nn.BatchNorm2d(F*D),
            nn.ELU(),
            nn.AvgPool2d((1, P1)),
            nn.Dropout(Pc),
...
class ClsHead(nn.Sequential):
    def __init__(self, linear_in, num_classes):
        super().__init__(
            nn.Flatten(),
            nn.Linear(linear_in, num_classes),
            nn.LogSoftmax(dim=1)
        )
...
class MSVTNet(nn.Module):
    def __init__(
        ...
        self.mstsconv = nn.ModuleList([
            nn.Sequential(
                TSConv(num_ch, F[b], C1[b], C2, D, P1, P2, Pc),
