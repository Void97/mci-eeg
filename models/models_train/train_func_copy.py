...
#### Here if the weights exist, no need to train again
...
def train_10_K_fold(seed, dirs, dataset_name, task, model_class, model_name, model_kwargs, best_params,
        dataset, samples, targets, groups, k, iter):
    
    all_test_preds       = []
    all_test_preds_roc   = []
    all_test_labels      = []
    all_test_subjects    = []
    subject_to_label     = {} #subject-label map
    k_fold_subject_logs  = [] #logs container
...
############################################################################    
    ...
    def tsne():
        ...
        # if dataset_name == "CAUEEG" and model_name == 'DeepConvNet':
        #     features, labels = subsample_then_svd(features, labels)
        #     print(f'Features subsampled shape: {features.shape}')
        #     print(f'Labels subsampled shape: {labels.shape}')
...
############################################################################
    ...
    # Build subject-to-label mapping (each subject should only map to one label!)
    set_seed(seed)
    ...
    for fold, (train_val_idx, test_idx) in enumerate(
        ...
        # inner_kf = StratifiedGroupKFold(n_splits=int(k)-1, shuffle=True, random_state=seed)
        ...
        one_fold_subject_logs = count_fold_subjects(groups, train_val_idx, 
                                                    train_idx, val_idx,
                                                    test_idx, subject_to_label,
                                                    fold)
        k_fold_subject_logs.append(one_fold_subject_logs)
        ...
        #### Create the weights dir
        best_model_dir = dirs['weights_dir'] 
        if not os.path.exists(best_model_dir):
            os.makedirs(best_model_dir)
        best_model_file_path = best_model_file_path = os.path.join(
            best_model_dir, 
        ...
        if os.path.exists(best_model_file_path):       
            print(f"Found existing weights for {dataset_name} {task} iteration {iter} fold {fold+1}.\n"
            ...
            for epoch in range(100):
                ...
                if val_acc > best_val_acc:
                    ...
                    counter += 1
                if counter >= patience:
                    print(f"Early stopping at epoch {epoch+1}. Loss: {val_loss}. Accuracy: {best_val_acc}. F1-score: {val_f1}\n Saving the model.")
                    break
        ...
        # if not os.path.exists(best_model_dir):
        #     os.makedirs(best_model_dir)
        # best_model_file_path = os.path.join(best_model_dir, f'{dataset_name}_{task}_{model_name}_iteration_{iter}_fold_{str(fold)}_best_weights.pth')
        ...
        if model_kwargs['num_classes'] == 2:
            print(f"Fold {fold+1} Test Acc: {accuracy_score(test_labels_fold, test_preds_fold):.4f}, "
                  f"F1: {f1_score(test_labels_fold, test_preds_fold):.4f}")
        else: 
            print(f"Fold {fold+1} Test Acc: {accuracy_score(test_labels_fold, test_preds_fold):.4f}, "
                  f"F1: {f1_score(test_labels_fold, test_preds_fold, average='weighted'):.4f}")
        ...
        test_subjects_fold = groups[test_idx].tolist()
        # test_preds.extend(test_preds_fold)
        all_test_preds.extend(np.array(test_preds_fold).tolist())
        all_test_preds_roc.extend(test_roc)
        all_test_labels.extend(test_labels_fold)
        all_test_subjects.extend(test_subjects_fold)
    ...
    with open(log_dir, 'w') as f:
        json.dump(k_fold_subject_logs, f, indent=2)

    return all_test_labels, all_test_preds, all_test_preds_roc, all_test_subjects, saliency_maps



def train_10_K_fold_pilot(seed, model_class, model_name, model_kwargs,
                          batch_sizes, lrs, L2s, 
